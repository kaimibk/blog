...
As we can already tell, there is already a massive speed boost. To make the point clearer, below is a figure where we inspect the average performance of each method for inputs of size $$[10^1, 10^2, 10^3, 10^4, 10^5, 10^6, 10^7, 10^8]$$ (generated using `np.logspace(1, 8, 8)`)&mdash;each experiment was repeated for 1000 trials.
...
Suppose we would like to compute the product of matrix, $$A$$, and a vector $$v$$: $$u = Av$$. That is to say, $$u_i - \sum_j A_{ij} v_j$$. The pseudocode may look like,
